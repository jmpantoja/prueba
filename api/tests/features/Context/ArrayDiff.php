...
use App\Behat\Context\Exception\ArrayDiffException;
...
class ArrayDiff
{
	...
	public function compare(array $needle, array $haystack): bool
	{
		...
		if ($needleIsList && !$haystackIsList) {
			throw new ArrayDiffException('The needle is a list, while the haystack is not.', $needle, $haystack);
		} elseif ($needleIsList && $haystackIsList) {
		...
		foreach ($needle as $key => $value) {
			...
			if (preg_match('/^(?<key>.*?)\[(?<index>[\d]+)\]$/', (string) $key, $match)) {
				...
				if (!empty($realKey) && !array_key_exists($realKey, $haystack)) {
					...
					throw new ArrayDiffException(sprintf('Haystack object is missing the "%s" key.', $realKey), $needle, $haystack);
				}
				...
				if (!is_array($subHaystack) || !$this->arrayIsList($subHaystack)) {
					...
					throw new ArrayDiffException(sprintf('The element at key "%s" in the haystack object is not a list.', $realKey), $needle, $haystack);
				} elseif (!array_key_exists($index, $subHaystack)) {
					// The index does not exist in the haystack
					throw new ArrayDiffException(sprintf('The index "%d" does not exist in the haystack list.', $index), $needle, $haystack);
				}

				if (is_array($value)) {
					if (!is_array($subHaystack[$index])) {
						throw new ArrayDiffException(sprintf('Expected haystack element on index "%d" to be an array, got "%s".', $index, gettype($subHaystack[$index])), $needle, $haystack);
					}
					...
					throw new ArrayDiffException(sprintf('Value mismatch for index "%d" in haystack list.', $index), $value, $subHaystack[$index]);
				}
			} else {
				...
				if (!array_key_exists($key, $haystack)) {
					// The key does not exist in the haystack
					throw new ArrayDiffException(sprintf('Haystack object is missing the "%s" key.', $key), $needle, $haystack);
				}

				if (is_array($value)) {
					if (!is_array($haystack[$key])) {
						throw new ArrayDiffException(sprintf('Expected haystack value with key "%s" to be an array, got "%s".', $key, gettype($haystack[$key])), $needle, $haystack);
					}

					// If the value is an array, recurse
					$this->compare($value, $haystack[$key]);
				} elseif (!$this->compareValues($value, $haystack[$key])) {
					throw new ArrayDiffException(sprintf('Value mismatch for key "%s" in haystack object.', $key), $needle, $haystack);
				}
			}
		}
	...
	protected function compareValues($needleValue, $haystackValue): bool
	{
		...
		if (is_string($needleValue) && $functions && preg_match($pattern, $needleValue, $match)) {
			...
			try {
				...
				throw new ArrayDiffException(sprintf('Function "%s" failed with error message: "%s".', $function, $e->getMessage()), $needleValue, $haystackValue);
			}
		}
			...
			if (is_array($needleValue)) {
				...
				if ($this->arrayIsList($needleValue)) {
					...
					if (empty($listElementsInHaystack)) {
						throw new ArrayDiffException('Haystack does not contain any list elements, needle can\'t be found.', $needleValue, $haystack);
					}
					...
					if (empty($result)) {
						throw new ArrayDiffException('The list in needle was not found in the list elements in the haystack.', $needleValue, $haystack);
					}
				} else {
					...
					if (empty($objectElementsInHaystack)) {
						throw new ArrayDiffException('Haystack does not contain any object elements, needle can\'t be found.', $needleValue, $haystack);
					}
					...
					// Result is empty, which means the needle was not found in the haystack
					if (empty($result)) {
						throw new ArrayDiffException('The object in needle was not found in the object elements in the haystack.', $needleValue, $haystack);
					}
				}
			} else {
				...
				if (empty(array_filter($result))) {
					throw new ArrayDiffException('Needle is not present in the haystack.', $needleValue, $haystack);
				}
			}
		}
